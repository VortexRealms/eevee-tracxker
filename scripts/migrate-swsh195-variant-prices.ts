/**
 * One-time migration: reassign swshp-SWSH195 price history from normal/holo → playPokemon,
 * fix current_prices variants_json (remove incorrect holo slot). Idempotent.
 *
 * Run with: npx tsx scripts/migrate-swsh195-variant-prices.ts
 */
...
const CARD_ID = "swshp-SWSH195";
const FROM_VARIANTS = ["normal", "holo"] as const;
const TO_VARIANT = "playPokemon";

export function migrateSwsh195VariantPrices(db: Database.Database): {
...
function main() {
  ...
  const result = migrateSwsh195VariantPrices(db);
  ...
  console.log(`  history before: normal=${before.normal}, holo=${before.holo}, playPokemon=${before.playPokemon}`);
  console.log(
    `  history renamed=${result.historyRenamed}, conflicts deleted=${result.historyConflictsDeleted}`
  );
  console.log(`  history after: normal=${after.normal}, holo=${after.holo}, playPokemon=${after.playPokemon}`);
